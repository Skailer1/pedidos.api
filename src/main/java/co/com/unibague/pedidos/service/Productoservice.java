...
public class Productoservice implements IProductoService
{
    ...
    @Override
    public Producto crear(Producto prodcuto) throws YaExisteEntidadExcepcion, DataIncorrectaExcepcion {
        if (!prodcuto.sonCamposValidos()) {
            throw new DataIncorrectaExcepcion("Verifique la información enviada");
       } else if (productoRepository.findById(prodcuto.getId()).isPresent()) {
            throw new YaExisteEntidadExcepcion("Ya existe un producto con ese id");
        } else {
            prodcuto.setFechaCreacion(new Date());
            prodcuto.setFechaActualizacion(new Date());
            prodcuto.setActivo(true);
            return productoRepository.save(prodcuto);
        }
    }
    ...
    public List<Producto> listarTodos() throws NoExisteEntidadExcepcion {
        ...
        if (productos.isEmpty()) {
            throw new NoExisteEntidadExcepcion("No hay producto registrados");
        } else {
            return productos;
        }
    }
